...
Daily approval and completion notices are triggered by the Codex/Claude project rules. Small/simple tasks do not send completion notices by default, so your phone is not flooded by tiny operations; only non-trivial tasks send a done notice before the final reply. After setup, you can send one optional test notice from the app to confirm that Feishu delivery works.
